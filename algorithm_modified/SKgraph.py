...
def _inf_mask(images, set, keep_set, keep_prob):
    ...
    weight_list = [W_conv1, W_fc1, W_fc2, W_fc3]
    middle_steps = [h_conv1, h_pool1, h_fc1, h_fc2]    
    return output, stitch, weight_list, middle_steps
...
def inference(images, set, keep_prob):
    ...
    if isinstance(set, int):
        ...
        output, _, weight_list, middle_steps = _inf_mask(images, set_tensor, set, keep_prob)
        return output, weight_list, middle_steps
    else:
        ...
        for keep_set in range(1, FLAGS.num_data_sets + 1):
            outputs[keep_set - 1], indices[keep_set - 1], _, _ = _inf_mask(images, set, keep_set, keep_prob)
