import algorithm.SKinput as input
...
class SetWriter(input.DirectoryEmbedded):
    ...
    def image_path(self):
        ...
        return self.file("images" + str(self.file_num) + ".txt")
    
    @property
    def info_path(self):
        # Path of current info file
        return self.file("info" + str(self.file_num) + ".txt")
...
class Separate(input.DirectoryEmbedded):
    ...
    def get_file_list(self):
        ...
        suffixes = [f[len("info"):] for f in os.listdir(self.directory) if f.startswith("info")]
        
        # Generate the list of file paths in the image_directory (image files can have prefixes
        image_files = [self.file((self.prefix  + "_" if self.prefix != "" else "") + "images" + suffix) for suffix in suffixes]
        info_files = [self.file("info" + suffix) for suffix in suffixes]
